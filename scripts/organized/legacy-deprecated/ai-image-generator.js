...
if (!showingHelp) {
  ...
  initScriptEnv(['AI_API_KEY', 'AI_API_ENDPOINT']);
}
...
class AIImageGenerator {
  constructor() {
    ...
    this.isGoogleAI = this.apiEndpoint && this.apiEndpoint.includes('generativelanguage.googleapis.com');
    this.isBananaAPI = this.apiEndpoint && this.apiEndpoint.includes('banana.dev');
  }
  ...
  async generateImage(prompt, options = {}) {
    ...
    try {
      ...
      if (this.isOpenAI) {
        ...
        const size = width >= 1024 ? '1024x1024' : width >= 512 ? '512x512' : '256x256';
